...
class Helper {
	...
	public static function getOtherKindShipString($id) {
		$kinships = array_column(\Yii::$app->params['other_kinship'], 'kinship_str', 'kinship_id');

		return isset($kinships[$id]) ? $kinships[$id] : '无';
	}
    ...
    /**
     * 货币进一保留prec位
     * @param  float  $x     规定要舍入的数字
     * @param  integer $prec 规定小数点后的位数
     * @return float         保留后的数
     */
	public static function currency($x, $prec = 0) {
		$i = pow(10, $prec);
		return ceil($x * $i) / $i;
	}
}